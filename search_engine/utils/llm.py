...
from search_engine.llm_provider.generic.base import NO_SUPPORT_TEMPERATURE_MODELS, SUPPORT_REASONING_EFFORT_MODELS, ReasoningEfforts
...
def get_llm(llm_provider, **kwargs):
    from search_engine.llm_provider import GenericLLMProvider
...
async def create_chat_completion(
    ...
    """Create a chat completion using the configured LLM provider API
    Args:
        ...
        reasoning_effort (str, optional): Reasoning effort for compatible models. Defaults to 'low'.
    ...
    # For Ollama, don't set max_tokens as it uses num_predict from llm_kwargs
    if model not in NO_SUPPORT_TEMPERATURE_MODELS:
        provider_kwargs['temperature'] = temperature
        # Only set max_tokens for non-Ollama providers
    ...
    # Apply llm_kwargs AFTER setting base parameters
    # For Ollama, this includes num_predict which is the correct parameter
    if llm_kwargs:
    ...
    if llm_provider == "ollama":
        ...
        # Debug: Log the actual parameters being passed to Ollama
        logging.info(f"Ollama parameters: model={provider_kwargs.get('model')}, num_predict={provider_kwargs.get('num_predict')}, temperature={provider_kwargs.get('temperature')}")

    provider = get_llm(llm_provider, **provider_kwargs)
    response = ""
    # create response
    for _ in range(10):  # maximum of 10 attempts
        # For Ollama, we don't pass generation parameters here - they're in the provider initialization
        # For other providers, we might need max_tokens
        generation_kwargs = {}
        if llm_provider != "ollama" and 'max_tokens' in provider_kwargs:
            generation_kwargs['max_tokens'] = provider_kwargs['max_tokens']
        generation_kwargs.update(kwargs)
        
        response = await provider.get_chat_response(
            messages, stream, websocket, **generation_kwargs
        )
