...
class GenericLLMProvider:
    ...
    def from_provider(cls, provider: str, chat_log: str | None = None, verbose: bool=True, **kwargs: Any):
        # REMOVED: OpenAI provider (Ollama-only setup)
        # if provider == "openai":
        ...
        # REMOVED: Azure OpenAI provider (Ollama-only setup)
        ...
        #     llm = AzureChatOpenAI(**kwargs)
            ...
            try:
                ...
                try:
                    ...
                    raise ImportError(
                        "Could not import 'ChatFireworks' from 'langchain_fireworks'. "
            ...
            # Remove base_url from kwargs if present to avoid duplicate parameter
            base_url = kwargs.pop("base_url", os.getenv("OLLAMA_BASE_URL", "http://localhost:11434"))
            
            # ChatOllama doesn't properly support num_predict in kwargs
            # We need to remove it here and hope it uses the model's default
            # OR we pass it in the actual API call (but that didn't work)
            
            # Extract and remove Ollama-specific params that ChatOllama doesn't handle well
            num_predict = kwargs.pop('num_predict', None)
            num_ctx = kwargs.pop('num_ctx', None)
            num_thread = kwargs.pop('num_thread', None)
            num_gpu = kwargs.pop('num_gpu', None)
            
            # Debug: Log what we're passing to ChatOllama
            import logging
            logging.info(f"[INIT] ChatOllama: model={kwargs.get('model')}, num_predict={num_predict} (removed from kwargs)")
            logging.info(f"[INIT] Remaining kwargs: {list(kwargs.keys())}")
            
            llm = ChatOllama(base_url=base_url, **kwargs)
            
            # Store num_predict in the llm object for later use
            if num_predict:
                llm._num_predict = num_predict
        ...
        # REMOVED: Dashscope provider (uses ChatOpenAI, Ollama-only setup)
        ...
        #         )
            ...
            llm = ChatXAI(**kwargs)
        # REMOVED: DeepSeek provider (uses ChatOpenAI, Ollama-only setup)
        ...
        #         )
            ...
            llm = GigaChat(**kwargs)
        # REMOVED: OpenRouter provider (uses ChatOpenAI, Ollama-only setup)
        ...
        #              **kwargs
        #         )
        # REMOVED: vllm_openai provider (uses ChatOpenAI, Ollama-only setup)
        ...
        #         **kwargs
        #     )
        # REMOVED: AIMLAPI provider (uses ChatOpenAI, Ollama-only setup)
    ...
    async def stream_response(self, messages, websocket=None, **kwargs):
        ...
        # Debug: Print kwargs to see if num_predict is present
        print(f"[DEBUG] stream_response called")
        print(f"[DEBUG] kwargs keys: {list(kwargs.keys())}")
        if 'num_predict' in kwargs:
            print(f"[DEBUG] num_predict = {kwargs['num_predict']}")
        ...
        print(f"[DEBUG] stream_response returning {len(response)} chars")
        return response
