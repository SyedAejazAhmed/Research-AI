...
async def generate_report(
    ...
    **kwargs
    ...
    try:
        logger.info(f"Calling LLM for report generation. Prompt length: {len(content)} chars")
        ...
        logger.info(f"LLM response received. Report length: {len(report) if report else 0} chars")
    except:
        logger.warning(f"First LLM attempt failed, trying alternative format...")
        try:
            report = await create_chat_completion(
                model=cfg.smart_llm_model,
                messages=[
            ...
            logger.info(f"Alternative format succeeded. Report length: {len(report) if report else 0} chars")
        except Exception as e:
